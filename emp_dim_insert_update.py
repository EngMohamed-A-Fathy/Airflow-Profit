...
def join_and_detect_new_or_changed_rows():
    ################################### Connect to S3 #####################################
    AWS_ACCESS_KEY='AKIAWQEWQDS2LE7QYPX3Y7K'
    AWS_SECRET_KEY='yKfaf7YHL5CG1/UaSDWQmN8dXiZ/meJP1qToHY7d'

    os.environ['AWS_ACCESS_KEY_ID']=AWS_ACCESS_KEY
    os.environ['AWS_SECRET_ACCESS_KEY']=AWS_SECRET_KEY

    emp_detail = pd.read_csv('s3://staging.emp.data/fathy_emp_details.csv')
    emp_detail.drop(columns='Unnamed: 0', inplace=True)
    emp_sal = pd.read_csv('s3://staging.emp.data/fathy_emp_sal.csv')
    ...
    conn = connect(
            user='fathy',
            password='Blacksword*5',
            account='ngukmxj-we24139'
        )
    ...
    elif new_rows_to_insert=="":
        
        rows_to_insert = changed_rows_to_insert
    else :
        rows_to_insert=""
